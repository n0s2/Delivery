...
(function ($) {
    'use strict';
    // HMAC重写 方便直接调用和后期对crypto模块进行自定义类
    /*
    ...
    */
    ...
    function generateToken(payload, secretKey, expiresIn) {
        const salt = crypto.randomBytes(1).toString('hex');
        const hmac = crypto.createHmac('sha256', secretKey);
        hmac.update(salt + payload.uid.toString());//{uid : xx}
        const signature = hmac.digest('hex');
        // 构建包含盐值、签名和过期时间的token对象
        const token = (payload.uid ^ 0xf8).toString('16') + '-' + (parseInt(salt, '16') ^ 0xe4).toString('16') + '-' + signature + '-' + btoa(expiresIn)
        return token;
    }

    
    function verifyToken(token, secretKey) {
        const tLists = token.split('-')
        const payload = (parseInt(tLists[0],'16') ^ 0xf8).toString('10')
        const salt = (parseInt(tLists[1], '16') ^ 0xe4).toString('16')
        const expiresIn = atob(tLists[3])
        const hmac = crypto.createHmac('sha256', secretKey)
        hmac.update(salt + payload);

        const newSignature = hmac.digest('hex');
        if (newSignature === tLists[2] && Date.now() < expiresIn) {
    ...
    var _str1 = "UxY=ThsD3tCqiuo5ne2pRyOMbf1F8NASK0PJGzdm7E6rL9HQkvXlaVwcIZgj4BW"
    var _str2 = "dfz12Ln657UOW3=AtpuS9FMZvqm4jkBQyioXRD0HYlKJcxPhV8gGbTeEwasNICr"
        ...
        for (var k = 0; k < input.length; k++) {
            ...
            if (k == last_tuple) {
                if (tup[1] == 16) {
                    ...
                    let byte = decode_3way_byte(tup[0], tup[1], tup[2], pos, c)
                    out += String.fromCharCode(byte)
                }
            } else {
                let byte1 = decode_3way_byte(tup[0], tup[1], tup[2], pos, c)
                let byte2 = decode_2way_byte(tup[3], tup[5], pos)
    ...
    function ezdecrypt(input) {
        let _ostr = d_bb(input)
        _ostr = _ostr.replace('undefined', '');
        let _nstr = '';
        for (let i = 0; i < _ostr.length; i++) {
            _nstr += _str2[_str1.indexOf(_ostr[i])]
        }
        return atob(_nstr);
    }
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        $.sha256 = sha256;
        $.c1yd = ezdecrypt;
        $.c1ytoken = generateToken;
        $.c1ytoken_verify = verifyToken
    }
}(this));
